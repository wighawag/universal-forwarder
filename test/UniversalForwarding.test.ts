...
import {ForwarderRegistry, UniversalForwarder, TestUniversalForwardingReceiver__factory} from '../typechain';
import {setupUsers} from './utils';
import {ForwarderRegistrySignerFactory, UniversalForwarderSignerFactory} from './utils/eip712';
...
const setup = deployments.createFixture(async () => {
	...
	const UniversalForwarder = <UniversalForwarder>await ethers.getContract('UniversalForwarder');
	const TestUniversalForwardingReceiverFactory = <TestUniversalForwardingReceiver__factory>(
...
describe('UniversalForwarding', function () {
	it('isTrustedForwarder', async function () {
		...
		expect(await ForwarderRegistry.isTrustedForwarder(ForwarderRegistry.address)).to.be.equal(true);
	});
	it('TestReceiver with msg.sender', async function () {
		...
		const data = await TestUniversalForwardingReceiver.callStatic.getData(users[0].address);
	...
	it('ForwarderRegistry metatx', async function () {
		const {users, TestUniversalForwardingReceiver, ForwarderRegistry, ForwarderRegistrySigner} = await setup();
		const {to, data} = await users[0].TestUniversalForwardingReceiver.populateTransaction.test(42);
		if (!(to && data)) {
		...
		const value = await TestUniversalForwardingReceiver.callStatic.getData(users[0].address);
		expect(value).to.equal(42);
	});

	it('UniversalForwarder metatx', async function () {
		const {users, TestUniversalForwardingReceiver, UniversalForwarder, UniversalForwarderSigner} = await setup();
		const {to, data} = await users[0].TestUniversalForwardingReceiver.populateTransaction.test(42);
		if (!(to && data)) {
			throw new Error(`cannot populate transaction`);
		}
		...
		const {data: relayerData} = await users[1].UniversalForwarder.populateTransaction.forward(
			signature,
			false,
			to,
			data
